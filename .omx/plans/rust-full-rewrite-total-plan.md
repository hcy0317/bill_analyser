...
Last updated: 2026-05-15T19:59:40+08:00.
...
| P14 | completed | `src/web/scripts/generate-rust-route-fixture.mjs`; `src/web/src/contracts/rustRouteOwnership.generated.ts`; `tests/web/contracts/frontendRustRouteContract.test.ts`; `src/web/src/components/BudgetManagementDialog.vue`; `src/web/src/lib/services.ts`; `docs/PROJECT_OVERVIEW.md`; `docs/overview-frontend.md`; `docs/overview-testing.md`; `npm run lint`; `npm run test:coverage`; `npm run build` | Frontend axios/fetch `/api/...` calls are now checked against the Rust ownership fixture and must resolve to `RustOwnedVerified` or `PythonDeleted` routes; stale `/api/budgets/status` and unused Python-proxied `llm/induce-rules` frontend references are removed; lint, coverage, and build gates pass. |

Next allowed phase by gate: P15 final Rust-only cutover/deletion; P15 must still prove no tracked Python, no proxy fallback, and Rust-only startup before completion.
