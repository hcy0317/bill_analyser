...
async def fix_missing_parents(dry_run: bool = False):
    """修复所有缺失的父级记录
...
async def main():
    ...
    try:
        await fix_missing_parents(dry_run=dry_run)
        
        if dry_run:
            logger.info("\n" + "=" * 80)
            logger.info("提示：使用以下命令执行实际修复：")
            logger.info("  .venv\\Scripts\\python scripts\\fix_missing_parent_categories.py")
            logger.info("=" * 80)
