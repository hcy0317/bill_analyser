...
if ($runBackend) {
    ...
    Invoke-RepoCommand "Rust coverage" "cargo llvm-cov --workspace --lcov --output-path workspace.lcov --fail-under-lines 35"
    Invoke-RepoCommand "Rust changed-line coverage" '$mergeBase = node scripts/resolve-ci-diff-refs.mjs read --input .omx/ultragoal/evidence/ci-diff-refs.json --field merge_base_sha; $head = node scripts/resolve-ci-diff-refs.mjs read --input .omx/ultragoal/evidence/ci-diff-refs.json --field head_sha; git -c gc.auto=0 diff --output=.omx/ultragoal/evidence/rust-changed.diff --unified=0 "${mergeBase}...${head}" -- src/backend tests/backend; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; node scripts/governance-normalizers.mjs changed-coverage --lcov workspace.lcov --diff .omx/ultragoal/evidence/rust-changed.diff --threshold 90 --require-matched-files --require-executable-lines'
}
