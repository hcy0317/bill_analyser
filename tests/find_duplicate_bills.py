...
try:
    from tests.runtime_paths import build_sqlite_uri, get_runtime_db_path
except ModuleNotFoundError:  # pragma: no cover - direct script execution fallback
    from runtime_paths import build_sqlite_uri, get_runtime_db_path


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="查找数据库中的重复账单")
    parser.add_argument("--db-path", default=None, help="目标数据库路径；删除模式下必须显式提供")
    parser.add_argument("--delete", action="store_true", help="删除重复账单（需显式提供 --db-path）")
    return parser
...
def main():
    ...
    db_path = Path(args.db_path).resolve() if args.db_path else get_runtime_db_path()

    if args.delete and args.db_path is None:
        print("错误: 删除模式请显式传入 --db-path，避免误删运行时数据库")
        sys.exit(2)
    ...
    conn = sqlite3.connect(build_sqlite_uri(db_path, readonly=dry_run), uri=True)
    try:
