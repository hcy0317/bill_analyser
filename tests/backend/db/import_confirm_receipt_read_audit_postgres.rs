...
async fn target_audit_covers_matching_receipts_in_bounded_read_only_batches(
    ...
    let result = async {
        ...
        assert_eq!(report.expected_migration_version, 42);
        assert_eq!(report.expected_migration_count, 42);
        assert_eq!(report.actual_migration_version, 42);
        assert_eq!(report.migration_count, 42);
