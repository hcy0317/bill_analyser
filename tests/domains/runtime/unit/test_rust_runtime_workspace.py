...
def test_architecture_docs_record_rust_primary_http_and_import_runtime_gates() -> None:
    ...
    assert "当前治理矩阵把第一阶段导入与预览决策旁路、核心账单 CRUD、交易图片上传/未使用清理、账单 CSV/XLSX 导出、账单 recurring candidates/match、账户对账单、账单分类 quick actions、账户 CRUD/display-order/余额同步/交易迁移清空、标签 CRUD/display-order/batch-create、分类主数据/分类账单统计/批量重分类、legacy 分类规则 config/cache、分类规则列表/创建/更新/删除/重排/defaults/migrate/测试、规则中心概览、设置包导入/预览/导出、预算 CRUD/export/execution/forecast/history/import、DB-backed statistics read routes、statistics Analyzer routes 与 statistics exchange routes 由 Rust runtime 接管且旧 Flask route shell 已删除" in architecture
    ...
    assert "真实 LLM provider 生成、全局 Learning Center suggestion/rules 决策与 receipt OCR recognition 均标为 Rust-owned，OAuth2 provider exchange 在当前 workspace build 中是 Rust-owned disabled-safe/not-implemented 合同" in architecture
    assert "显式 `import_db_runtime` 已能用 Rust 处理 import v2 JSON parse/parse_generic/dedup/confirm、前端 FormData CSV/XLS/XLSX/HTML-xls 上传专用 parser 检测解析、未匹配文件 temp preview 与列映射 parse_generic、session/preview 读取清理、preview update/reclassify 显式 DB 更新、preview-item transfer/recurring 决策、learning 会话预览旁路、LLM provider 推荐/分析/规则归纳、LLM accept/reject/memory 事件、全局 Learning Center suggestions/rules loop、OCR config app_settings 与 receipt recognition、账单 CRUD、交易图片上传/未使用清理、账单 CSV/XLSX 导出、账单 recurring candidates/match、账户对账单、账单分类 quick actions、账户 CRUD/display-order/余额同步/交易迁移清空、标签 CRUD/display-order/batch-create、分类主数据与 `GET /api/categories/statistics`/`POST /api/categories/update-all`、legacy `GET|PUT /api/categories/rules` config/cache、分类规则创建/更新/删除/重排/defaults/migrate、设置包导入/预览/导出、预算 CRUD/export/execution/forecast/history/import，以及 category statistics/trends、asset trends、category pie、top merchants、amounts 与 Analyzer overview/trends/comparison/category/trend 统计读取与 exchange-rate provider/custom-rate runtime" in architecture
    ...
    assert "导入 session、preview、parser template staging、StandardBill-to-parser-template adapter、parser-template-to-preview staging adapter、selected preview confirm-to-bills、preview update/reclassify、preview-item transfer/recurring/learning decision、learning promotion、preview LLM recommendation/memory event、全局 Learning Center suggestion mining/rule store、OCR config app_settings 与 annotation sample" in project_overview
    ...
    assert "旧 Flask bills import route package 已删除，Python sidecar 不再注册 `/api/bills`" in project_overview
    assert "preview update/reclassify、preview-item transfer/recurring/learning decision、learning promotion、preview LLM recommendation/memory event、全局 Learning Center suggestions/rules loop、OCR config app_settings、annotation sample upsert/read" in architecture
    assert "对应 Flask sidecar bills import/read/exchange/Analyzer/insights route shell 已删除" in architecture
    ...
    assert "P11 provider execution makes Rust the runtime owner for live LLM provider generation in `import_db_runtime`: `POST /api/llm/preview-recommend`, `POST /api/llm/analyze-transactions`, and `POST /api/llm/rule-synthesis`" in migration_plan
...
def test_migration_governance_matrix_tracks_current_import_preview_adjacent_python_routes() -> None:
    ...
    bills_route_files = list(bills_routes_dir.glob("*.py")) if bills_routes_dir.exists() else []
    assert not bills_route_files
    for path in bills_route_files:
    ...
    learning_routes_path = REPO_ROOT / "src/bill_analyser/api/routes/learning.py"
    if learning_routes_path.exists():
        for method, route_path in _extract_flask_route_pairs(learning_routes_path):
