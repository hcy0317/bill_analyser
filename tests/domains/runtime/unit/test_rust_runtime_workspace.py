...
def test_architecture_docs_record_rust_primary_http_and_import_runtime_gates() -> None:
    ...
    assert "当前治理矩阵把第一阶段导入与预览决策旁路、核心账单 CRUD、交易图片上传/未使用清理、账单 CSV/XLSX 导出、账单 recurring candidates/match、账户对账单、账单分类 quick actions、账户 CRUD/display-order、标签 CRUD/display-order/batch-create、分类主数据/分类账单统计/批量重分类、分类规则列表/测试、规则中心概览、设置包导出、预算 CRUD/export/execution/forecast/history/import、DB-backed statistics read routes 标为 Rust-owned" in architecture
    ...
    assert "同时把账户交易 move/clear 与余额同步、统计 Analyzer overview/trends/comparison/category/trend、真实 exchange-rate provider/custom-rate 写入、真实 LLM/OCR provider 生成、settings bundle import/preview、全局 Learning Center suggestion/rules 决策与真实 OAuth provider exchange 标为 Python-proxied" in architecture
    assert "显式 `import_db_runtime` 已能用 Rust 处理 import v2 JSON parse/parse_generic/dedup/confirm、前端 FormData CSV 上传解析、未匹配文件 temp preview 与列映射 parse_generic、session/preview 读取清理、preview update/reclassify 显式 DB 更新、preview-item transfer/recurring 决策、learning 会话预览旁路、LLM accept/reject/memory 事件、OCR config app_settings、账单 CRUD、交易图片上传/未使用清理、账单 CSV/XLSX 导出、账单 recurring candidates/match、账户对账单、账单分类 quick actions、账户 CRUD/display-order、标签 CRUD/display-order/batch-create、分类主数据与 `GET /api/categories/statistics`/`POST /api/categories/update-all`、设置包导出、预算 CRUD/export/execution/forecast/history/import，以及 category statistics/trends、asset trends、category pie、top merchants、amounts 统计读取" in architecture
    ...
    assert "当前通过显式 `import_db_runtime` 暴露 import v2 JSON parse/parse_generic/dedup/confirm、前端 FormData CSV 上传解析、未匹配文件 temp preview 与列映射 parse_generic、session/preview 读取清理、preview update/reclassify 显式 DB 更新、preview-item transfer/recurring 决策、learning 会话预览旁路与 promotion、LLM accept/reject/memory 事件、OCR config app_settings、账单 CRUD、交易图片上传/未使用清理、账单 CSV/XLSX 导出、账单 recurring candidates/match、账户对账单、账单分类 quick actions、账户 CRUD/display-order、标签 CRUD/display-order/batch-create、分类主数据、分类账单统计、批量重分类、设置包导出、模板 CRUD/display-order、预算 CRUD/export/execution/forecast/history/import 与统计读取" in architecture
