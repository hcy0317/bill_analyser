...
def test_architecture_docs_record_rust_primary_http_and_import_runtime_gates() -> None:
    ...
    assert "当前通过显式 `import_db_runtime` 暴露 import v2 JSON parse/parse_generic/dedup/confirm、前端 FormData CSV 上传解析、未匹配文件 temp preview 与列映射 parse_generic、session/preview 读取清理、preview update/reclassify 显式 DB 更新、preview-item transfer/recurring 决策、learning 会话预览旁路与 promotion、LLM accept/reject/memory 事件、OCR config app_settings、账单 CRUD、交易图片上传/未使用清理、账单 CSV/XLSX 导出、账单 recurring candidates/match、账户对账单、账单分类 quick actions、账户 CRUD/display-order/余额同步/交易迁移清空、标签 CRUD/display-order/batch-create、分类主数据、分类账单统计、批量重分类、legacy 分类规则 config/cache、分类规则创建/更新/删除/重排/defaults/migrate、设置包导入/预览/导出、模板 CRUD/display-order、预算 CRUD/export/execution/forecast/history/import、统计读取与汇率 REST；对应 Flask sidecar read/exchange route shell 已删除" in architecture
