...
def test_bills_preview_transfer_decision_route_preserves_response_mode_compatibility(
    bills_route_app: Flask,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """preview-item transfer 决策路由仅在显式带 responseMode 时才透传该参数。"""
    db = FakeBillsDB()
    service = FakeBillsService()
    category_engine = FakeBillsCategoryEngine()
    loop = FakeLoop()
    ...
    route = _unwrap_all(bills_module.update_preview_transfer_decision)

    compatibility_calls: list[tuple[int, str, dict[str, Any], int]] = []

    async def _apply_without_response_mode(
        preview_id: int,
        decision: str,
        *,
        expected_state: dict[str, Any],
        ...
        compatibility_calls.append((preview_id, decision, dict(expected_state), user_id))
        return {
            "success": True,
            "preview_id": preview_id,
            "session_id": "sess-preview",
            "decision": decision,
            "preview_item": {"id": preview_id},
            "preview": [{"id": preview_id}],
        }

    monkeypatch.setattr(service, "apply_preview_transfer_decision", _apply_without_response_mode, raising=False)
    with bills_route_app.test_request_context(
        "/api/bills/import/v2/preview-item/1/transfer-decision",
        method="POST",
        json={"decision": "accept", "expectedState": {"sessionId": "sess-preview"}},
    ):
        _set_request_user_id(7)
        payload = route(1).get_json() or {}
        assert payload["success"] is True
        assert payload["data"]["previewItem"] == {"id": 1}
        assert compatibility_calls == [(1, "accept", {"sessionId": "sess-preview"}, 7)]

    explicit_response_mode_calls: list[tuple[int, str, dict[str, Any], str | None, int]] = []

    async def _apply_with_response_mode(
        preview_id: int,
        decision: str,
        *,
        expected_state: dict[str, Any],
        ...
        explicit_response_mode_calls.append(
            (preview_id, decision, dict(expected_state), response_mode, user_id)
        )
        return {
            "success": True,
            "preview_id": preview_id,
    ...
    monkeypatch.setattr(service, "apply_preview_transfer_decision", _apply_with_response_mode, raising=False)
    with bills_route_app.test_request_context(
        "/api/bills/import/v2/preview-item/1/transfer-decision",
        method="POST",
        json={
            "decision": "clear",
            "expectedState": {"sessionId": "sess-preview"},
        ...
        payload = route(1).get_json() or {}
        assert payload["success"] is True
        assert explicit_response_mode_calls == [
            (1, "clear", {"sessionId": "sess-preview"}, "preview-item", 8)
        ]
