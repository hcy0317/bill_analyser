...
from src.core.investment_settings import (
    build_user_investment_keyword_settings,
    serialize_keyword_list,
)
from src.utils.constants import FRONTEND_TO_BACKEND_TYPE
...
def generate_access_token(user_id: int, username: str, config: dict,
                          expires_in_seconds: int = 0,
                          token_kind: str = 'session') -> dict:
...
def _build_auth_success_result(user: dict, tokens: dict,
                               application_cloud_settings: Optional[List[dict]] = None) -> dict:
    """构建统一的认证成功响应结构。"""
    return {
...
def _generate_recovery_codes() -> List[str]:
    """生成恢复码列表。"""
    return [
        f"{secrets.token_hex(4)[:4]}-{secrets.token_hex(4)[:4]}".upper()
        for _ in range(8)
    ]
...
def _parse_export_datetime(raw_value: str) -> Optional[str]:
        ...
        for category_id in category_ids:
            ...
            if category:
                selected_categories.append({
                    'main': category.get('main_category', ''),
...
def _render_bills_export(
    ...
    writer.writerow([
        'id', 'date', 'type', 'amount', 'main_category', 'sub_category',
        'source_account', 'destination_account', 'counterparty',
        'payment_method', 'description', 'tags', 'comment',
...
def decode_action_token(token: str, config: dict, expected_type: str) -> Optional[dict]:
    """解码并校验动作令牌。"""
    if not token:
        return None
...
def login():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'result': _build_auth_success_result(user_info, tokens, application_cloud_settings)
        })
...
def register():
    ...
    try:
        ...
        preset_categories_saved = loop.run_until_complete(
            _save_register_categories(db, user_id, register_categories)
        )
    ...
    try:
        ...
        if payload.get('email') != email:
            return jsonify({
                'success': False,
                'error': 'Invalid token',
                'message': 'Reset password token does not match email'
            }), 400
        ...
        password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        loop.run_until_complete(db.update_user(user['id'], {
            'password_hash': password_hash
        }))
        loop.run_until_complete(db.create_auth_log({
            'user_id': user['id'],
...
def authorize_oauth2_callback():
    ...
    try:
        ...
        if not config.get('enable_oauth2', False):
            return jsonify({
                'success': False,
                'error': 'OAuth2 disabled',
                'message': 'OAuth2 login is currently disabled'
            }), 403
...
def profile():
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            logger.info(f"返回用户资料: user_id={request.user_id}, username={user_info['username']}, "
        ...
        if 'investmentPlatformKeywords' in data:
            update_data['investment_platform_keywords'] = serialize_keyword_list(
                data['investmentPlatformKeywords']
            )
        if 'investmentProductKeywords' in data:
            update_data['investment_product_keywords'] = serialize_keyword_list(
                data['investmentProductKeywords']
            )
        if 'investmentExcludeKeywords' in data:
            update_data['investment_exclude_keywords'] = serialize_keyword_list(
                data['investmentExcludeKeywords']
            )
...
def update_profile_avatar():
    ...
    try:
        ...
        success = loop.run_until_complete(db.update_user(request.user_id, {
        ...
        return jsonify({
            'success': True,
            'result': _build_user_profile_info(user)
        })
...
def unlink_profile_external_auth():
    ...
    try:
        ...
        existing = loop.run_until_complete(
            db.get_user_external_auth(request.user_id, external_auth_type)
        )
        if not existing:
            loop.close()
            return jsonify({
        ...
        success = loop.run_until_complete(
            db.delete_user_external_auth(request.user_id, external_auth_type)
        )
        loop.run_until_complete(db.create_auth_log({
            'user_id': request.user_id,
            'username': request.username,
...
def _generate_personal_token(token_kind: str):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': 'Invalid request',
            'message': 'expiresInSeconds must be a valid integer'
        }), 400
    except Exception as e:
        if loop and not loop.is_closed():
            loop.close()
...
def revoke_token(token_id: str):
    ...
    try:
        ...
        result = loop.run_until_complete(
            db.invalidate_session_by_id(session_id, request.user_id)
        )
        loop.close()
...
def list_tokens():
    ...
    try:
        ...
        for session in sessions:
            ...
            last_seen = _datetime_to_unix_millis(
                session.get('last_activity_at') or session.get('created_at', '')
            )
...
def profile_cloud_settings():
    ...
    try:
        ...
        loop.run_until_complete(
            db.update_user_application_cloud_settings(
                request.user_id,
                normalized_settings,
    ...
    try:
        ...
        if not secret or not passcode:
            return jsonify({
                'success': False,
                'error': 'Bad Request',
                'message': 'Secret and passcode are required'
            }), 400
    ...
    try:
        ...
        if normalized_file_type not in ['csv', 'tsv']:
            return jsonify({
                'success': False,
                'error': 'Invalid request',
                'message': 'Unsupported export file type'
            }), 400
...
def clear_user_transactions():
    ...
    try:
        ...
        loop.run_until_complete(db.create_audit_log(
