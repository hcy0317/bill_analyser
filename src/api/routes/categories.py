...
def create_category():
    ...
    try:
        ...
        if str(parent_id) == '0':
            ...
            # 检查分类是否已存在
            existing = loop.run_until_complete(db.get_category_by_name(main_category_name, name, user_id=request.user_id))
            if existing:
                loop.close()
    ...
    try:
        ...
        if str(category_id).startswith('virtual_'):
            ...
            if success:
                ...
                result = {
                    'id': str(updated_cat['id']),
                    'name': updated_cat['sub_category'] if updated_cat['sub_category'] else updated_cat['main_category'],
                    'parentId': '0',
                    'type': updated_cat.get('type', 1),
                    'icon': updated_cat.get('icon', ''),
                    'color': updated_cat.get('color', ''),
    ...
    try:
        ...
        for item in new_display_orders:
            ...
            if cat_id and display_order is not None:
                loop.run_until_complete(db.update_category(int(cat_id), {'priority': display_order}, user_id=request.user_id))
    ...
    try:
        ...
        for cat in categories:
            ...
            for sub in sub_categories:
                ...
                existing_sub = loop.run_until_complete(db.get_category_by_name(main_name, sub_name, user_id=request.user_id))
                if not existing_sub:
    ...
    try:
        ...
        if str(category_id).startswith('virtual_'):
            ...
            try:
                ...
                if cat['sub_category']:
                    # 查找是否存在对应的主分类记录
                    parent = loop.run_until_complete(db.get_category_by_name(cat['main_category'], '', user_id=request.user_id))
                    if parent:
