<template>
    <f7-page @page:afterin="onPageAfterIn">
        <f7-navbar>
            ...
            <f7-nav-right>
                <f7-link :class="{ 'disabled': !fromAccount || !toAccountId || fromAccount?.id === toAccountId || !toAccountName || !isToAccountNameValid || !password || moving }" :text="tt('Confirm')" @click="confirm"></f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        <f7-list form strong inset dividers class="margin-vertical" v-if="!loading">
            ...
            <f7-list-input
                ...
                clear-button
                :label="tt('Password')"
                :placeholder="tt('Enter password to confirm this operation')"
...
function confirm(): void {
    ...
    if (!fromAccount.value || !toAccountId.value || fromAccount.value?.id === toAccountId.value || !toAccountName.value || !isToAccountNameValid.value || !password.value) {
        return;
    }
