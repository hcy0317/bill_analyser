...
export const useTransactionCategoriesStore = defineStore('transactionCategories', () =>{
    ...
    const hasAvailableInvestmentCategories = computed<boolean>(() => {
        if (!allTransactionCategories.value || !allTransactionCategories.value[CategoryType.Investment] || !allTransactionCategories.value[CategoryType.Investment].length) {
            return false;
        }

        const firstAvailableCategoryId = getFirstAvailableCategoryId(allTransactionCategories.value[CategoryType.Investment]);
        return firstAvailableCategoryId !== '';
    });
