...
export const useAccountsStore = defineStore('accounts', () => {
    ...
    function loadAllAccounts({ force }: { force: boolean }): Promise<Account[]> {
        ...
        loadAllAccountsPromise = new Promise((resolve, reject) => {
            services.getAllAccounts({
                ...
                // 即使数据相等，也应正常返回，这不是错误情况
                if (force && data.result && isEquals(allAccounts.value, accounts)) {
                    logger.info('Account list is already up to date, no changes needed');
                    resolve(accounts); // 改为resolve而非reject
                    return;
                }
     ...
     *
     * @example
     * // 输入: ['3', '52'] (农业银行主账户, 中信建投证券子账户)
     * // 输出: ['203', '204', '52'] (农业银行-储蓄卡, 农业银行-信用卡, 中信建投证券)
     *
     * 用途：账户筛选时，选择主账户应该包含其所有子账户的交易
     */
    function expandAccountIds(accountIds: string[]): string[] {
        const result = new Set<string>();

        logger.info(`[expandAccountIds] 输入账户ID: [${accountIds.join(', ')}]`);

        for (const accountId of accountIds) {
            const account = allAccountsMap.value[accountId];

            if (!account) {
                logger.warn(`[expandAccountIds] 未找到账户: ${accountId}`);
                continue;
            }
            ...
            if (account.type === AccountType.MultiSubAccounts.type) {
                ...
                if (account.subAccounts && account.subAccounts.length > 0) {
                    for (const subAccount of account.subAccounts) {
                        result.add(subAccount.id);
                        logger.debug(`[expandAccountIds] 展开子账户: ${subAccount.id} (${subAccount.name})`);
                    }
                } else {
                    // 主账户但没有子账户，保留主账户ID
                    result.add(accountId);
                    logger.debug(`[expandAccountIds] 主账户无子账户，保留: ${accountId}`);
                }
            } else {
                // 单一账户或已是子账户，直接添加
                result.add(accountId);
                logger.debug(`[expandAccountIds] 添加单一账户: ${accountId} (${account.name})`);
            }
        }

        const expandedIds = Array.from(result);
        logger.info(`[expandAccountIds] 输出账户ID: [${expandedIds.join(', ')}] (共${expandedIds.length}个)`);
