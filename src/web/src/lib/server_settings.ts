function getServerSetting(key: string): string | number | boolean | Record<string, string> | undefined | null {
    const settings = window.bill_analyser_SERVER_SETTINGS || {};
    return settings[key];
}

function isEnabledSetting(key: string): boolean {
    return getServerSetting(key) === 1;
}
...
export function isTransactionFromAIImageRecognitionEnabled(): boolean {
    return isEnabledSetting('llmt') || isEnabledSetting('air');
}
