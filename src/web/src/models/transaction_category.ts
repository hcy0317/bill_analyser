...
export class TransactionCategory implements TransactionCategoryInfoResponse {
    ...
    public static of(json: any): TransactionCategory {
        ...
        let subCategories: TransactionCategory[] | undefined = undefined;

        if (json.subCategories && Array.isArray(json.subCategories)) {
            subCategories = [];

            for (const subCategoryJson of json.subCategories) {
                subCategories.push(TransactionCategory.of(subCategoryJson));
            }
        }
