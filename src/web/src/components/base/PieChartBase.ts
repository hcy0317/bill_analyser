...
export function usePieChartBase(props: CommonPieChartProps) {
    ...
    const validItems = computed<CommonPieChartDataItem[]>(() => {
        ...
        for (const item of props.items) {
            ...
            if (isNumber(value) && Math.abs(value) > 0 && (!props.hiddenField || !item[props.hiddenField])) {
                totalValidValue += Math.abs(value);
            }
        }
        ...
        for (const item of props.items) {
            ...
            if (isNumber(value) && Math.abs(value) > 0 &&
                (!props.hiddenField || !item[props.hiddenField]) &&
                (!props.minValidPercent || Math.abs(value) / totalValidValue > props.minValidPercent)) {
                const finalItem: CommonPieChartDataItem = {
                    ...
                    percent: (isNumber(percent) && percent >= 0) ? percent : (Math.abs(value) / totalValidValue * 100),
                    actualPercent: Math.abs(value) / totalValidValue,
