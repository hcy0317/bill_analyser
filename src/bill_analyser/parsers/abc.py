...
class ABCParser(ParserBase):
    ...
    def _get_counterparty(self, row_dict: dict[str, str]) -> str:
        ...
        counterparty = (
            row_dict.get("对方户名")
            or row_dict.get("对方名称")
            or row_dict.get("对⼿信息")
            or row_dict.get("对方账号")
            or ""
        )
    ...
    def _find_header_row(self, df: pd.DataFrame) -> int:
        ...
        for i in range(min(15, len(df))):
            row_values = [str(df.iloc[i, j]) for j in range(len(df.columns)) if not pd.isna(df.iloc[i, j])]
    ...
    def _check_abc_content(self, content: str) -> bool:
        """检查内容是否符合农业银行特征"""
        # 农业银行强特征标识
        abc_strong_indicators = ["中国农业银行股份有限公司", "农业银⾏", "账⼾活期交易明细清单"]

        # 农业银行特有的列名模式（旧格式：特殊编码）
        abc_old_format_columns = ["⼾名", "账⼾", "交易⽇期", "交易⾦额", "对⼿信息", "本次余额"]
        ...
        # 排除其他银行的特征
        other_bank_indicators = ["支出金额", "存入金额", "凭证类型", "储种", "记账日"]
