"""Parser tag normalization helpers."""
...
def normalize_parser_tags(raw_tags: Any) -> list[str]:
    ...
    if isinstance(raw_tags, str):
        ...
        try:
            ...
            return _dedupe_tags(
                [_normalize_tag_value(part) for part in text.split(",") if str(part).strip()]
            )

    if isinstance(raw_tags, (list, tuple, set)):
        return _dedupe_tags(
            [_normalize_tag_value(item) for item in raw_tags if _normalize_tag_value(item)]
        )
...
def _detect_channel_tag(parser_id: str, payment_method: str, channel: str) -> str:
    ...
    if any(
        keyword in combined_text for keyword in ("wechat", "微信", "alipay", "支付宝", "零钱", "wallet")
    ):
        return "wallet"
...
def resolve_parser_tags(
    ...
    *,
    ...
    return build_parser_tags(parser_id=parser_id, payment_method=payment_method, channel=channel)
