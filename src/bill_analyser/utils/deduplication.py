...
class DeduplicationEngine:
    ...
    def _find_transfer_pairs(self, bills: list[dict[str, Any]]) -> list[tuple[dict[str, Any], dict[str, Any]]]:
        ...
        for i, bill1 in enumerate(bills):
            ...
            # 检查是否包含转账关键词
            has_transfer_keyword = any(keyword in bill1_desc or keyword in bill1_desc for keyword in transfer_keywords)
            ...
            for j, bill2 in enumerate(bills[i + 1 :], start=i + 1):
                ...
                # 检查是否一方支出一方收入
                if (bill1_amount > 0 and bill2_amount > 0) or (bill1_amount < 0 and bill2_amount < 0):
                    continue
                ...
                # 检查描述相似度或包含转账关键词
                has_bill2_keyword = any(keyword in bill2_desc or keyword in bill2_desc for keyword in transfer_keywords)
