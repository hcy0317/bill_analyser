...
from bill_analyser.utils.constants import BACKEND_TO_FRONTEND_TYPE, FRONTEND_TO_BACKEND_TYPE
...
class TransactionAdapter:
    ...
    async def backend_to_frontend(
        ...
        result: dict[str, Any] = {
            ...
            "categoryName": str(bill.get("main_category", "") or ""),
            "subCategoryName": str(bill.get("sub_category", "") or ""),
            "time": time_value,
            ...
            "amount": yuan_to_cents(abs(amount)),
    ...
    async def backend_list_to_frontend(
        self, bills: list[dict[str, Any]], total: int, page: int, page_size: int
    ) -> dict[str, Any]:
        """批量转换账单列表。"""
        ...
        items = [await self.backend_to_frontend(bill, account_map, category_map) for bill in bills]
