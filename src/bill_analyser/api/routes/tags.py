...
def get_tags():
    ...
    try:
        ...
        tags = _run_async(db.get_all_tags(user_id=user_id))

        return jsonify({"success": True, "result": tags})

    except Exception as exc:  # pylint: disable=broad-exception-caught
        logger.error("获取标签列表失败: %s", exc)
...
def get_tag(tag_id: int):
    ...
    try:
        db = get_app_context()
        user_id = _get_request_user_id()
        tag = _run_async(db.get_tag_by_id(tag_id, user_id=user_id))
...
def create_tag():
    ...
    try:
        ...
        db = get_app_context()
        user_id = _get_request_user_id()
        tag_id = _run_async(db.create_tag(data, user_id=user_id))
        tag = _run_async(db.get_tag_by_id(tag_id, user_id=user_id))

        return jsonify({"success": True, "result": tag or {"id": tag_id}}), 201

    except Exception as exc:  # pylint: disable=broad-exception-caught
        logger.error("创建标签失败: %s", exc)
        return jsonify({"success": False, "error": str(exc)}), 500
...
def update_tag(tag_id: int):
    ...
    try:
        ...
        db = get_app_context()
        user_id = _get_request_user_id()
        result = _run_async(db.update_tag(tag_id, data, user_id=user_id))
        tag = None
        if result:
            tag = _run_async(db.get_tag_by_id(tag_id, user_id=user_id))
...
def delete_tag(tag_id: int):
    ...
    try:
        db = get_app_context()
        user_id = _get_request_user_id()
        result = _run_async(db.delete_tag(tag_id, user_id=user_id))
...
def create_tags_batch():  # pylint: disable=too-many-return-statements
    ...
    try:
        ...
        if not isinstance(tags_data, list) or not tags_data:
            return jsonify(
                {"success": False, "error": "tags is required and must be a non-empty array"},
            ), 400
        ...
        created_tags = []
        existing_tags = _run_async(db.get_all_tags(user_id=user_id))
        existing_by_name = {
            str(tag.get("name", "")).strip().lower(): tag
        ...
        for item in tags_data:
            if not isinstance(item, dict) or not str(item.get("name", "")).strip():
                return jsonify(
                    {"success": False, "error": "Each tag item must contain a non-empty name"},
                ), 400
            ...
            if normalized_name in existing_by_name:
                if skip_exists:
                    ...
                    continue
                return jsonify(
                    {"success": False, "error": f"Tag already exists: {item['name']}"},
                ), 409

            tag_id = _run_async(db.create_tag(item, user_id=user_id))
            created_tag = _run_async(db.get_tag_by_id(tag_id, user_id=user_id))
            if created_tag:
        ...
        logger.error("批量创建标签失败: %s", exc, exc_info=True)
        return jsonify({"success": False, "error": str(exc)}), 500
    ...
    try:
        ...
        for item in new_orders:
            ...
            try:
                ...
                return jsonify({
                    "success": False,
                    "error": f"Invalid id or displayOrder: {exc}",
                }), 400

        user_id = _get_request_user_id()
        logger.info("[标签排序更新] 开始: user_id=%s, count=%s", user_id, len(orders))

        db = get_app_context()
        result = _run_async(db.update_tag_display_orders(orders, user_id=user_id))

        if result:
            logger.info("[标签排序更新] 完成: user_id=%s, count=%s", user_id, len(orders))
            return jsonify({"success": True, "result": True})
        return jsonify({"success": False, "error": "Failed to update display orders"}), 500
    except Exception as exc:  # pylint: disable=broad-exception-caught
        logger.error("更新标签显示顺序失败: %s", exc, exc_info=True)
        return jsonify({"success": False, "error": str(exc)}), 500
