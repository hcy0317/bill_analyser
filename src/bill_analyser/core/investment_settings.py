...
DEFAULT_INVESTMENT_PLATFORM_KEYWORDS: List[str] = [
    '蚂蚁财富', '天天基金', '理财通', '东方财富', '同花顺', '雪球',
    '基金销售平台', '证券账户', '余额宝', '招财宝', '国债逆回购',
    '京东金融', '肯特瑞', '且慢', '蛋卷基金', '陆金所', '度小满理财',
    '招银理财', '工银理财', '建信理财', '中银理财', '农银理财',
    '华泰证券', '招商证券', '中信证券', '广发证券', '国泰君安',
    '富途证券', '老虎证券'
]

DEFAULT_INVESTMENT_PRODUCT_KEYWORDS: List[str] = [
    '基金', '理财', '定投', '申购', '赎回', '买入', '卖出',
    '货币基金', '指数基金', '债券基金', '混合基金', 'REITs', 'REIT',
    'ETF', 'LOF', 'QDII', '债券', '股票', '黄金', '可转债',
    '固收+', '现金管理', '养老目标', '国债', '逆回购', '组合', '计划'
]

DEFAULT_INVESTMENT_EXCLUDE_KEYWORDS: List[str] = [
    '还款', '借呗', '花呗', '贷款', '信用卡', '房贷', '车贷',
    '待还款', '分期', '账单', '生活缴费', '水费', '电费', '话费'
]
...
def build_user_investment_keyword_settings(user: Optional[Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    return {
        'platform_keywords': normalize_keyword_list(
            user.get('investment_platform_keywords'),
            DEFAULT_INVESTMENT_PLATFORM_KEYWORDS
        ),
        'product_keywords': normalize_keyword_list(
            user.get('investment_product_keywords'),
            DEFAULT_INVESTMENT_PRODUCT_KEYWORDS
        ),
        'exclude_keywords': normalize_keyword_list(
            user.get('investment_exclude_keywords'),
            DEFAULT_INVESTMENT_EXCLUDE_KEYWORDS
        )
    }
