...
class SmartDeduplicationEngine:
    ...
    DEFAULT_INVESTMENT_KEYWORDS = [
        '基金', '理财', '投资', '股票', '债券', '定期',
        '余额宝', '零钱通', '黄金', '保险', 'fund', 'invest',
        '蚂蚁财富', '蚂蚁基金', '博时黄金', '国泰黄金', '天弘',
        'etf', 'lof', '指数', '货币基金', '定投'
    ]

    DEFAULT_TRANSFER_KEYWORDS = [
        '转账', '转入', '转出', '汇款', 'transfer',
        '银行转入', '银行卡转', '跨行转账', '同行转账'
    ]
    ...
    async def load_keywords_from_db(self, db, user_id: int = 1) -> None:
        ...
        try:
            ...
            for cat in categories:
                ...
                if cat_type == TransactionType.TRANSFER:
                    ...
                    self.logger.debug(
                        ...
                        cat.get('main_category'), cat.get('sub_category'),
            ...
            self.logger.info(
                ...
                len(self._investment_keywords), len(self._transfer_keywords)
            )
    ...
    def process(self, bills: List[Dict[str, Any]]) -> DeduplicationResult:
        ...
        self.logger.info("[去重完成] 原始: %d, 保留: %d, 移除: %d",
                        original_count, len(kept_bills), result.removed_count)
    ...
    async def process_with_db(
        ...
        self.logger.info("[去重完成(含数据库)] 原始: %d, 保留: %d, 移除: %d",
                        original_count, len(kept_bills), result.removed_count)
    ...
    def _find_platform_bank_duplicates(self, bills: List[Dict[str, Any]]) -> List[DuplicateGroup]:
        ...
        # 分离平台账单和银行账单
        platform_bills = [b for b in bills if b.get('source_account_id') in platform_sources
                         and not b.get('_removed')]
        bank_bills = [b for b in bills if b.get('source_account_id') in bank_sources
                     and not b.get('_removed')]
        ...
        for p_bill in platform_bills:
            ...
            for i, b_bill in enumerate(bank_bills):
                ...
                if (self._time_close(p_dt, b_dt, self.TIME_TOLERANCE_PLATFORM_BANK) and
                    self._amount_equal(p_amt, b_amt) and
                    same_direction):
                    ...
                    groups.append(DuplicateGroup(
                        ...
                        keep_bill=p_bill,
                        remove_bills=[b_bill],
                        reason=(f"支付平台({p_bill.get('source_account_id')})"
            ...
            for j in range(i + 1, n):
                ...
                if is_same_source_exact_match:
                    self.logger.debug(
                        ...
                        source1, time_diff, cp1[:30], cp2[:30], similarity
                    )
                    # 不检查相似度，直接进入匹配逻辑
                else:
                    # 非同源：排除投资配对场景
                    if source1 != source2:
                        ...
                        if is_investment:
                            self.logger.debug(
                                ...
                                source1, source2, desc1[:30], desc2[:30]
                            )
                            continue
                    ...
                    if similarity < 0.5:
                        self.logger.debug(
                            "[相似度去重] 不匹配: %s vs %s, 相似度=%.2f < 0.5",
                            cp1[:30], cp2[:30], similarity
                        )
                        continue
                ...
                groups.append(DuplicateGroup(
                    ...
                    reason=(f"相似度去重: 时间差{time_diff:.0f}秒, "
                ...
                self.logger.info(
                    "[相似度去重] 匹配成功: "
                    "时间差=%.0f秒, 金额=%.2f, 相似度=%.2f, "
                    "保留=%s, 移除=%s",
                    time_diff, amt1, similarity,
                    source1, source2 if primary_bill is bill1 else source1
                )
    ...
    def _find_paired_transactions(
        ...
        self.logger.debug(
            ...
            len(investment_keywords), investment_keywords[:5] if len(investment_keywords) > 5 else investment_keywords,
            len(transfer_keywords), transfer_keywords[:5] if len(transfer_keywords) > 5 else transfer_keywords
        )
    ...
    def _find_split_bills(self, bills: List[Dict[str, Any]]) -> List[Dict]:
        ...
        active_bills = [(i, b) for i, b in enumerate(bills)
    ...
    async def find_database_duplicates(
        ...
        for bill in bills:
            ...
            for eb in candidates:
                ...
                desc_similar = (bill_desc and eb_desc and
                               (bill_desc in eb_desc or eb_desc in bill_desc))
                counterparty_similar = (bill_counterparty and eb_counterparty and
                                       (bill_counterparty in eb_counterparty or
                                        eb_counterparty in bill_counterparty))
                ...
                if desc_similar or counterparty_similar or (not bill_desc and not eb_desc):
                    ...
                    groups.append(DuplicateGroup(
                        ...
                        keep_bill=eb,
                        remove_bills=[bill],
                        reason=(f"与数据库已有账单重复 (ID={eb.get('id')}, "
                               f"日期={eb.get('date')}, 金额={eb.get('amount')})")
                    ))
                    self.logger.debug(
                        "[数据库重复] 新账单 %s/%s 与已有账单 ID=%s 重复",
                        bill_date, bill_amt, eb.get('id')
                    )
                    break
        ...
        for target_name, keywords in investment_targets.items():
            for keyword in keywords:
                if keyword in combined:
                    self.logger.debug("[投资目标] 从 '%s' 提取到目标: %s",
                                    combined[:50], target_name)
                    return target_name
