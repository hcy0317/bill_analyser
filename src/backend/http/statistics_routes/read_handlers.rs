...
use bill_analyser_db::{
    find_postgres_statistics_all_date_range, query_asset_trends_payload,
    query_category_pie_payload, query_category_statistics_payload, query_category_trends_payload,
    query_postgres_asset_trends_payload, query_postgres_category_pie_payload,
    query_postgres_category_statistics_payload, query_postgres_category_trends_payload,
    query_postgres_top_merchants_payload, query_postgres_transaction_amount_period,
    query_top_merchants_payload, query_transaction_amount_period, StatisticsBillFilters,
};
use serde_json::json;
...
use super::{
    query::{
        asset_date_range_from_all_range, asset_date_range_from_query,
        asset_date_range_from_timestamp_range, bill_filters_for_timestamp_range,
        date_from_timestamp, default_year_month_query_values, non_empty_string,
        timestamp_range_from_query, year_month_range_from_query, year_month_range_from_values,
        BasicStatisticsQuery, CategoryStatisticsQuery, CategoryTrendsQuery,
    },
    response::{
        bad_request, db_error_response, internal_error, open_postgres_runtime, open_runtime,
...
pub(super) async fn category_statistics_handler(
    ...
    if state.config.database_backend.uses_postgres() {
        ...
        return match query_postgres_category_statistics_payload(runtime.pool(), user_id, &filters)
            .await
        {
...
pub(super) async fn category_trends_handler(
    ...
    if state.config.database_backend.uses_postgres() {
        ...
        return match query_postgres_category_trends_payload(
            runtime.pool(),
            user_id,
            &filters,
            &range,
...
pub(super) async fn asset_trends_handler(
    ...
    if state.config.database_backend.uses_postgres() {
        ...
        return match query_postgres_asset_trends_payload(
            runtime.pool(),
            user_id,
            start_date,
            end_date,
