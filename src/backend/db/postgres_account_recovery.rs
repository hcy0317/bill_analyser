...
const OPTIONAL_LEGACY_USER_DATA_DELETE_TABLES: &[&str] = &["account_aliases_legacy"];
...
async fn delete_target_user_business_rows(
    ...
    for table_name in OPTIONAL_LEGACY_USER_DATA_DELETE_TABLES {
        if postgres_table_exists(transaction, table_name).await? {
            let sql = format!("DELETE FROM {table_name} WHERE user_id = $1");
            sqlx::query(&sql)
                .bind(target_user_id)
                .execute(&mut **transaction)
...
mod tests {
    ...
    #[tokio::test]
    async fn recovery_delete_business_rows_clears_optional_legacy_alias_table_when_present(
    ) -> DbResult<()> {
        ...
        let target_user_id = 9_102_002_i64;
        let mut transaction = pool.begin().await?;
        sqlx::query(
            r#"
        ...
        delete_target_user_business_rows(&mut transaction, target_user_id).await?;
