...
pub async fn clear_postgres_user_data(
    ...
    for table_name in ["account_aliases_legacy"] {
        delete_postgres_user_rows_if_table_exists(&mut transaction, table_name, user_id).await?;
    }
    transaction.commit().await?;
    Ok(UserDataClearAllResult { counts })
}
...
async fn delete_postgres_user_rows_if_table_exists(
    transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    table_name: &'static str,
    user_id: i64,
) -> DbResult<()> {
    let exists = sqlx::query_scalar::<_, bool>("SELECT to_regclass($1) IS NOT NULL")
        ...
        .await?;
    if exists {
        delete_postgres_user_rows(transaction, table_name, user_id).await?;
    }
    Ok(())
}
...
mod tests {
    ...
    #[tokio::test]
    async fn postgres_user_data_clear_removes_optional_legacy_alias_table_when_present(
    ) -> DbResult<()> {
        ...
        let user_id = 9_102_001_i64;
        let mut transaction = pool.begin().await?;
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS account_aliases_legacy (
        ...
        .execute(&mut *transaction)
        .await?;
        sqlx::query("INSERT INTO account_aliases_legacy (user_id, value) VALUES ($1, 'legacy')")
            .bind(user_id)
            .execute(&mut *transaction)
            .await?;

        delete_postgres_user_rows_if_table_exists(
            &mut transaction,
        ...
        let remaining = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*)::BIGINT FROM account_aliases_legacy WHERE user_id = $1",
        )
        .bind(user_id)
        .fetch_one(&mut *transaction)
        .await?;
