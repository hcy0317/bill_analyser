...
- `crates/bill-analyser-core/src/statistics.rs` 当前固定统计/汇率/净值/洞察/日历的 Rust 合同层，覆盖反向区间 400、分/元换算、分类/账户聚合、资产趋势余额、空账户图例过滤、Analyzer period window 与 overview/trends/comparison/category/trend 聚合、汇率 provider/custom/fallback、净值分组、洞察异常和日历 projection shape；`import_db_runtime` 已由 Rust 直接执行 DB-backed statistics read、Analyzer statistics 与汇率 REST，旧 Flask statistics read/exchange/Analyzer route shell 已删除，统计 report/chart generation 仍由 Python Flask/aiosqlite 保留。
...
- Flask sidecar 中 `/api/statistics/category-statistics*`、`/asset-trends`、`/category-pie`、`/top-merchants`、`/amounts`、Analyzer `/overview`、`/trends`、`/comparison`、`/category`、`/trend` 与 `/exchange-rates*` 不再注册；`statistics.bp` 仅作为空 package 兼容出口保留
