...
- Flask sidecar 中 `/api/statistics/category-statistics*`、`/asset-trends`、`/category-pie`、`/top-merchants`、`/amounts`、Analyzer `/overview`、`/trends`、`/comparison`、`/category`、`/trend`、`/exchange-rates*` 与 `/api/insights/anomalies` 不再注册；`statistics.bp` 与 `insights.bp` 仅作为空 package 兼容出口保留
