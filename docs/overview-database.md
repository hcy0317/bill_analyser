...
- user-scope 查询与写入。
- 导入 session/source/standard-row/template/preview staging 只保存当前导入过程所需临时数据；source 与 standard row 台账记录 parser decision、标准化行和后续决策分组锚点，preview page 查询负责按条件分页和返回轻量 facets/counts；confirm、cancel、失败后新建 session 会清理对应用户的 import staging，未完成导入不作为可续传数据保留。
