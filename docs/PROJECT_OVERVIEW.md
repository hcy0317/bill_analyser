...
正式交易前端当前以入口 facade + 功能子文件组织：`src/web/src/stores/transaction.ts` 保留 `useTransactionsStore` facade，筛选/month-list 类型和 receipt draft 归一化 helper 下沉到 `stores/transaction/**`；`src/web/src/models/transaction.ts` 保留 `Transaction`/`TransactionGeoLocation` facade，REST request/response、统计、overview 和空结果合同下沉到 `models/transaction/contracts.ts`；桌面交易列表、桌面编辑弹窗、批量手工录入、移动列表和移动编辑页保留原 `.vue` 入口，template/style 及无状态展示 helper 下沉到相邻功能文件夹。该拆分不改变 Pinia store 导出名、模型 import 路径、桌面/移动交易入口、投资/转账字段、图片 OCR、批量录入或筛选 URL 合同。
