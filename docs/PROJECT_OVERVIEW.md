...
- `category_engine.py`：分类规则表达式解析与分类匹配（含类型过滤与预编译优化）；运行时仅从 `category_rules` canonical source 加载规则，不再回退到 `categories.keywords`；`rule_expression` 后端支持旧 `OR:a|b&AND:c&NOT:d`、新 `OR={a,b}+AND={c}+NOT={d}`，并通过 AST 执行括号分组与表达式级 `|` 优先级；迁移旧关键词到新表达式时会转义逗号、加号、花括号、竖线等字面量分隔符，避免特殊字符关键词被拆成错误语义。
...
- `desktop/pairingcenter/ListPage.vue` 当前承载桌面端 canonical 规则中心：页面展示为“Rule Center / 规则中心”，以转账配对、投资配对、长期学习、LLM 识别四个一级入口组织持久化匹配治理、学习规则与识别配置。
- `/pairing/list` 仍保留为既有深链兼容路径，但运行态会使用 `domain/tab` query 表达规则中心信息架构；`/rules/center` 与 `/learning/center` 只重定向到同一规则中心页面，不再保有独立持久化业务中心语义。
...
- 前端 `src/web/src/views/desktop/transactions/import/tabs/ImportTransactionCheckDataTab.vue` 当前也是用户触发“会话级学习助手”的主入口：用户在导入预览里选择 preview 行，调用 `getImportLearningSuggestions` / `promoteImportLearning` 来审核并提升长期学习规则；`src/web/src/views/desktop/learningcenter/ListPage.vue` 仅保留 legacy route wrapper 语义，相关 LLM / learning 能力由规则中心 `/pairing/list?domain=learning|llm&tab=...` 承接。
- `POST /api/llm/analyze-transactions` 当前也支持导入预览会话输入：请求体可携带 `session_id` 与 `preview_updates[]` / `preview_ids[]`，服务端会先同步当前 preview 草稿，再基于该 `import_session` 的 `bills_preview` 分组生成 `rule_induction` 型 `llm_candidates`。默认推荐入口是导入预览会话助手；legacy Learning Center route 只会重定向到规则中心的 learning / LLM domain。
