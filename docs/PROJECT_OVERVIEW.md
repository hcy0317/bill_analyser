...
- `desktop/pairingcenter/ListPage.vue` 当前承载桌面端 canonical 规则中心：一级导航分为“配对总览”“规则配置”“长期学习”“LLM 识别”。“配对总览”下有“转账配对 / 投资配对”，读取并管理历史正式账单 pair；“规则配置”下有“分类识别 / 投资识别 / 周期识别”，分别承接 category rules、investment recognition settings 与 recurring matching 入口；“长期学习”下保留自动建议与学习规则；“LLM 识别”下保留 LLM 规则候选与 LLM 配置。页面以预算管理式纵向导航 + 二级 tabs 展示当前域，右侧标题栏统一承载当前二级标题、刷新/新增/生成建议/筛选等动作，正文子组件不再重复渲染同名标题；分类识别表格的批量选择、表头筛选、每页数量与分页统计都收敛在表头/分页区，避免切换多选时推动整表布局。
...
- 导入预览 `Check Data` 当前把交易类型职责与匹配信号职责拆开：类型列只负责展示/编辑 `preview_type` 以及与类型直接相关的编辑状态；`Signals` 列集中展示解析器来源、去重/转账、只读投资提示、长期学习与定期交易等匹配信号，人工标注状态只通过行首编辑/标注入口和标注筛选表达，不再作为 `Signals` 列 chip 展示。`transfer / learning / recurring` 继续映射为可操作候选；`investment` 在导入预览中只作为只读提示 chip 展示 platform/product/profile 信息，不再暴露 accept / reject / clear 操作。普通银行结息、投资收益/分红/亏损等余额变化类流水会在导入分类前转为普通收入/支出并抑制 investment signal；`parser / dedup` 作为上下文摘要显示，跨解析器来源会串联为类似 `匹配 | 民生银行 | 支付宝` 的提示。预览确认与重新分类请求结构不因这层展示消费而改变。
- 投资识别链路只把真正的投资交易/配对候选提升为投资类型；普通银行结息通过 `is_ordinary_bank_interest_income()` 归一为收入，投资收益/分红/亏损通过 PnL 判断归一为收入或支出，并清理投资候选字段与同账户目标账户，避免把“余额变化”继续当作投资信号或原路转账。账户余额因此按普通收入/支出口径变化，而不是通过同账户投资流水抵消。
