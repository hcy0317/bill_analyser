...
- 投资识别链路只把真正的投资交易/配对候选提升为投资类型；可编辑的投资识别关键词会迁入投资分类规则表达式，运行时分类引擎命中投资类型分类规则时会把账单类型提升为“投资”。普通银行结息通过 `is_ordinary_bank_interest_income()` 归一为收入，投资收益/分红/亏损通过 PnL 判断归一为收入或支出，并清理投资候选字段与同账户目标账户，避免把“余额变化”继续当作投资信号或原路转账。账户余额因此按普通收入/支出口径变化，而不是通过同账户投资流水抵消。
