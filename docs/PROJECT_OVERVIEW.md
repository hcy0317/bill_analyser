...
导入预览并发补充：`selection_hash` 是当前用户、当前 session 完整选择集合的唯一集合 token，`row_version` 只保护单行业务字段，二者不互相递增。显式 `selectionAction=patch` 在 active-session 锁事务内校验完整已选 id 快照，再原子应用 selected/deselected 两组更新；条件选择则在同一锁事务内先校验 `selection_hash`，再逐行校验 `preview_updates.expected_row_version` 并写草稿，最后按更新后的数据库状态执行 query selection。任一集合或行 token 过期都会在事务提交前失败并整体零写入，分别返回 `409 PREVIEW_SELECTION_CONFLICT` 的权威 metadata/目标行快照，或 `409 PREVIEW_ROW_VERSION_CONFLICT` 的最新冲突行；成功响应返回本次草稿涉及的完整 `previewItems`。纯 selection writer 只更新 selected、兼容 JSON 投影与 `updated_at`，不推进 `row_version`；只有业务草稿 patch 推进行版本。无 token 的旧调用保持兼容。桌面跨页选择全部通过中立 typed services facade 发送当前集合 token 与可用的行 token；成功后用完整行回执重基，selection 冲突重基集合与目标行，row 冲突只重基冲突行并保留同批其他未写草稿，所有冲突都终止当前动作而不自动重试。集合 token 不会被隐式加入 confirm 或 LLM 生成 payload。
