...
- matching 域当前还提供 `POST /api/matching/candidates/<candidate_id>/reject` 作为 generic reject 的当前切片：它现在支持 `preview:<preview_id>:transfer` 与 `preview:<preview_id>:recurring`。`preview:<preview_id>:transfer` 分支继续复用导入预览写接口的 `reject` 语义，并要求请求体携带 `expectedState(sessionId, reviewStatus, previewType, categoryId, recurringId)` 以保持防陈旧写保护；成功后返回 `data{candidateId, action, previewId, sessionId, preview[]}`。被拒绝的 transfer candidate 仍会保留在 session 级读模型中，但状态会投影为 `rejected`。`preview:<preview_id>:recurring` 分支则复用 preview recurring clear 写路径，要求请求体携带 `expectedState(sessionId, previewType, categoryId, recurringId)`；成功后同样返回 `data{candidateId, action, previewId, sessionId, preview[]}`，并清除当前 preview 上的 recurring 绑定。该 recurring candidate 在 session 级读模型中会继续保留，但状态会从 `confirmed` 回到 `pending`；这是一种 preview family 级 reject/clear 语义，而不是按具体 `recurring_id` 的 suppressed 状态。`bill:<billId>:transfer:<candidateBillId>` 与其他 family 当前仍不支持 generic reject，并统一返回 `400 Candidate family not supported`，等待后续 feedback / suppression model 与 reconcile-history 切片补齐。
- matching 域当前还提供历史正式账单 transfer-only 后配对 MVP：`GET /api/matching/bills/<bill_id>/candidates` 会按当前用户返回 `data{billId, linkedPair, candidates[]}`，其中 `linkedPair` 在账单已配对时包含 `id / pairType / source / leftBillId / rightBillId / otherBillId`，未配对时为 `null`；`candidates[]` 仅投影同用户、未参与其他 pair、金额绝对值相等且方向相反、来源账户不同、且时间落在窗口内的 transfer 候选，并为每项返回稳定 `candidateId=bill:<billId>:transfer:<candidateBillId>`、`billId / score / level / reason / bill{...}`。对应写接口 `POST /api/matching/manual-pair` 使用 `{ billId, candidateBillId }` 建立 1:1 手工配对：`self-pair` 返回 `400`，账单不存在或不属于当前用户返回 `404`，重复/反向重复/任一账单已参与其他 transfer pair、或两条账单不满足 transfer-only 配对约束时返回 `409`。删除接口 `DELETE /api/matching/pairs/<id>` 仅删除当前用户下的正式账单 transfer/manual pair：pair 不存在或不属于当前用户返回 `404`，成功时返回被删除的 `data{pair}` 快照；删除后再次读取 `GET /api/matching/bills/<bill_id>/candidates` 会恢复 `linkedPair=null` 并重新投影候选。列表接口 `GET /api/matching/pairs` 会按当前用户返回 `data{pairs[]}`，只读取 `bill_pair_links` 中已持久化的 transfer/manual pair，并为每条 pair 附带 `id / pairType / source / leftBillId / rightBillId / createdAt / updatedAt` 以及 `leftBill / rightBill` 两侧正式账单的最小摘要，供后续 pairing center 或审查入口消费。该能力当前以 `bill_pair_links` 作为最小持久化模型，只覆盖正式账单、transfer-only、manual source，不包含 preview 写路径、investment pairing 或 feedback event 表；相关 pair link 会在显式删除 pair、单条删除、批量删除、清空用户交易和清空用户业务数据时同步清理。
