...
- `category_engine.py`：分类规则表达式解析与分类匹配（含类型过滤与预编译优化）；运行时仅从 `category_rules` canonical source 加载规则，不再回退到 `categories.keywords`；`rule_expression` 后端支持旧 `OR:a|b&AND:c&NOT:d`、新 `OR={a,b}+AND={c}+NOT={d}`，并通过 AST 执行括号分组与表达式级 `|` 优先级。
