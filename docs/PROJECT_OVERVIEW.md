...
- `db_import_configs.py` / `db_import_sessions.py` / `db_import_preview.py` / `db_import_learning.py`：导入模板配置、三阶段会话、预览编辑/确认、长期学习 durable corpus、exact/manual 学习规则、session-scoped dry-run learning suggestions 与复合匹配特征
...
- 导入学习域当前分为 durable corpus、exact/manual overlay 和 session dry-run 三层：`Database.save_import_annotation_samples(...)` 会继续写 session 临时表 `import_annotation_samples`，同时把同一 preview 的 parser-aware `composite` 特征（`parser_id / counterparty / description / payment_method`）与人工标注写入 `import_learning_corpus_samples`；`clear_session_data()` 只清理 parser / preview / session annotation 临时数据，不删除 durable corpus，因此 Learning Center 的历史挖掘仍可跨 session 使用长期语料。`GET /api/bills/import/v2/learning/<session_id>/suggestions` 仍读取当前 session 的 `import_annotation_samples` 与 `bills_preview` 生成 dry-run suggestions；`POST` 入口可先保存 `preview_updates[]` 并按 `previewIds[]` 裁剪本次选择。正式提升继续写入 `import_learning_rules`，该表是 exact/manual overlay 真值层，并通过 `composite_match_hash / match_features_json` 驱动运行时匹配；编辑 composite `matchValue` 时后端会重算这些运行时字段。Learning Center 的跨 session 挖掘使用 `import_learning_corpus_samples` 生成 `import_learning_suggestions`，accept / reject / preview apply / rollback / auto apply 会写 `import_learning_feedback_events` 与 `import_learning_concept_stats` 作为可观测事件和计数底座。当前导入预览结构 map 为 `ImportTransactionCheckDataTab.vue -> ImportLearningSuggestionDialog.vue -> services.ts -> api/routes/bills.py -> BillService.get_import_learning_suggestions(...)/promote_session_annotations_to_learning(...) -> Database.save_import_annotation_samples(...)/promote_import_annotation_samples_to_learning(..., preview_ids=...)`。
