...
分页草稿只覆盖相对服务端 baseline 的字段差量：先把差量叠加到权威行，再从 effective row 重算分类/账户 review；服务端值与草稿值相同时视为 mutation ack 并收敛 baseline，未回执字段继续保留临时 overlay。因此旧 `matching` 不会在翻页时复活已解决的 missing-category，已确认字段也不会成为长期前端脏缓存。
