...
learning 与 LLM 审核进入各自独立事务；LLM 的导入专用审核路由和 matching candidate action 复用同一 repository 事务。事务锁定 preview row，校验 expected state，持久化合法 lifecycle transition，并把 committed、idempotent、stale/conflict 结果返回现有 REST envelope。两类审核都可携带行版本前置条件：同一 terminal action 的重放先走既有幂等返回，其余过期 token 在任何 preview、lifecycle 或 LLM memory 写入前回滚，并由 HTTP 层回读当前 user/session 范围内的最新完整行返回 `409 PREVIEW_ROW_VERSION_CONFLICT`。桌面与移动调用方消费该快照重建行状态及下一次 token；旧调用方不传 token 时保持原有兼容行为。语义 no-op 编辑保留 pending LLM；接受或拒绝会退出 pending；实质编辑和 reclassify 通过统一 invalidation 路径清理依赖旧字段的 learning、history、LLM 与辅助 recurring 状态。confirm 不重复执行已由独立审核事务持久化的 effect。
