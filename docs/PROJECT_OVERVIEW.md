...
显式 selection patch 的 selected/deselected 两组在一个 session 锁事务内提交，跨组重复和 session 外 id 失败关闭；`selection_hash` 只承担当前用户、当前 session 完整选择集合的并发前置条件，不复用或替代 `row_version`。条件选择仍保留原有 query-target 事务边界。
