...
导入预览后端当前以 facade + 功能子文件组织：`src/backend/db/import_staging.rs` 聚合 session、parser/source staging、preview query/predicate/selection/write/identity/confirm/materialization/row mapping/filter matching 等子文件；`src/backend/db/import_stage2.rs` 统一持有 Stage 2 所需分类、规则、账户、learning 与 recurring 上下文的 user-scoped SQL；`src/backend/http/import_routes/stage_handlers.rs`、`preview_mutation_helpers.rs` 与 `stage_vector_recall.rs` 聚合 dedup/stage2、preview page/selection/update/reclassify、confirm、learning、vector recall 等子文件；`src/backend/core/import_pipeline.rs` 聚合导入预览 pipeline 合同，preview page query、filter index、matching payload、类型映射、route response DTO、response envelope 和 value helper 下沉到 `import_pipeline/**`。初始导入、直接重分类和 decision-group 重分类统一通过 `ImportStage2::evaluate` 进入应用边界；该边界每批只加载一次当前用户权威上下文并构建不跨请求复用的 immutable snapshot，再按既有固定顺序执行分类、recurring、learning 与账户规则投影。`PreviewStateKernel::derive` 接受 typed facts 并输出带 projection version 的 `SignalSet`、`ReviewIssueSet`、decision evidence view 与 effective identity；现有轻量 filter index 先通过集中 legacy adapter 消费该 snapshot，再从同一 snapshot 回投旧 transfer/learning/LLM/history 状态字段。该边界不改变现有 REST 路由或 PostgreSQL reader；legacy `import_preview_signal_flags` 只作为迁移期只读 shadow oracle，不获得新规则或写权。
