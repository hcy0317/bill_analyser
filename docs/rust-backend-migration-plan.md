...
S15c.2 makes Rust the runtime owner for `GET /api/category-rules/`. The Rust path reads `category_rules` joined to `categories` through `bill-analyser-db`, keeps authenticated `user_id` scope, preserves the Flask-compatible `success/data/total` envelope, and supports `category_id` plus `enabled_only=false` query semantics. Category rule mutations/test/defaults/migrate/reorder, settings bundle import, plus account transaction move/clear/balance sync routes remain Python-proxied until separate P4 follow-ups port those side effects.
