...
账号业务数据恢复使用独立工具 `bill_postgres_account_recovery`，不复用整库导入命令。该工具面向单个 SQLite 源用户和显式 Postgres 目标用户，默认只做只读 preflight/dry-run；`apply` 必须带 manifest id、目标用户三元确认和 git-ignored snapshot 目录，才会在单个目标用户事务中清空并重建业务表。`apply` 默认继续保留 Postgres 目标用户认证，并把 legacy 账户别名同时保留在账户 metadata 与派生 `account_rules` 中；若目标账号是为恢复临时创建且需要沿用 legacy SQLite 登录密码，必须单独运行 `sync-auth`，它只同步源用户 password hash 并清理目标锁定计数，不重建业务数据。Postgres URL 优先从进程内的 `BILL_ANALYSER_POSTGRES_URL` 读取，避免把带密码的连接串放进命令行参数。
...
cargo run -p bill-analyser-db --bin bill_postgres_account_recovery -- --mode sync-auth --sqlite data\backups\bills-pre-import-staging-cleanup-20260517_190849.db --source-user-id 5 --target-user <id-or-username-or-email> --confirm-target-user-id <id> --confirm-target-username <username> --confirm-target-email <email> --output .git\ai\recovery-snapshots\auth-sync.redacted.json
```
...
- `sync-auth` requires the same target id/username/email confirmation, locks the target user row, copies only the source SQLite `users.password_hash` into the target PostgreSQL user, clears `failed_login_attempts` and `locked_until` metadata, and records only redacted hash-presence/audit evidence. It does not change usernames, emails, 2FA, sessions, external auth, settings, or business rows.
...
- Legacy SQLite account `aliases` are retained in account metadata for the account settings UI and are also converted directly into `account_rules` with `source=sqlite_account_recovery`.
