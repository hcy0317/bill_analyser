...
- 账户、交易分类、交易标签、交易模板、定时交易、规则中心分类识别与 LLM 配置页面各自提供设置 JSON 导入入口：页面标题栏只显示一个 `Import` 按钮，悬停 1.5 秒后在同一按钮菜单卡片中显示 `Export Settings JSON`；导入会读取本地 JSON、调用对应 section 的预览接口汇总新增/更新/跳过数量，确认后只导入当前页面对应的设置 section 并刷新本页数据。数据管理页不再作为这些设置 JSON 的唯一入口。
