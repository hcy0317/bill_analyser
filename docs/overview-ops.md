...
- 云同步 REST 入口 `POST /api/backup/sync` 由 Rust 创建本地备份后直接上传到 OSS/S3/COS/Azure Blob/WebDAV；对象前缀会先规整为安全相对路径并补齐尾随 `/`，绝对路径、Windows drive、反斜杠、`..` 段和控制字符会被拒绝；endpoint 默认只允许官方云厂商 HTTPS 域名，WebDAV、自托管 S3-compatible 或内网 NAS 目标必须通过 `BILL_ANALYSER_BACKUP_SYNC_ENDPOINT_ALLOWLIST` 显式按 origin 放行，metadata/link-local 目标始终拒绝；响应、审计与 `backup_records` metadata 只保留递归脱敏后的 provider 配置。Python `SyncManager` 仅作为 CLI/兼容残留，不再是默认 backup ops REST 运行时依赖。
