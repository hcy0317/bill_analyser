...
- Rust 运维合同层位于 `crates/bill-analyser-core/src/ops.rs`，当前覆盖日志/运维邻近的备份、加密状态、同步配置和报告导出格式等纯合同逻辑；Python 仍是实际日志线程、文件 I/O、云 SDK 上传与报表文件生成运行时。

## 7.1.1 备份、加密与同步
- 备份 REST 主链继续由 `src/bill_analyser/api/routes/backup/` 提供；恢复预验证和实际恢复都会拒绝包含绝对路径或 `..` 段的 zip member，且只把包含 `data/` 目录的安全 zip 视为可恢复，Rust 合同层固定安全文件名、`.zip.enc` 加密备份识别、恢复预验证摘要、metadata checksum 投影和 cleanup retention 决策，避免 Python route 与 DB 记录排序/保留语义漂移。
