...
- Rust 主 HTTP 服务在 `import_db_runtime` 模式下接管无外部 provider 依赖的统计读取入口：
  ...
  - 旧 Flask `statistics` read/exchange route shell 已删除；Flask sidecar 仅继续注册 Analyzer 代理路由；
