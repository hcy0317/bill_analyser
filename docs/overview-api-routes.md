...
- Rust 主 HTTP 服务在 `import_db_runtime` 模式下接管 recurring suggestions、calendar 与 networth 入口：
  - Rust-owned：`GET /api/recurring/suggestions`、`POST /api/recurring/suggestions/detect`、`POST /api/recurring/suggestions/<suggestion_id>/accept`、`POST /api/recurring/suggestions/<suggestion_id>/reject`、`GET /api/calendar/events`、`GET /api/networth/snapshot`；
  - Python-proxied：formal matching candidates、feedback、manual pairs、reconcile history 与 investment settings 仍在 `/api/matching/*` sidecar 边界内；
  - 周期建议检测复用 Rust matching detector 并持久化 `recurring_suggestions`；accept 写入 `recurring_bills`，calendar 返回账单日聚合和 recurring projection，networth 按账户类型分组资产/负债；旧 Flask `recurring.py`、`calendar.py` 与 `networth.py` route shell 已删除。
...
- 主要 Python REST route 文件当前按同名 package 组织：`auth/`、`statistics/`、`backup/`、`matching/`、`llm/` 均保留原 `bp` 导出与 URL/method 契约；学习中心、账户、分类、分类规则、标签、模板、设置包、预算、账单、recurring suggestions、calendar 与 networth REST 主链由 Rust runtime 提供，不再保留 Flask `accounts/` / `categories/` / `budgets/` / `bills/` route package、`category_rules.py`、`tags.py`、`templates.py`、`settings_bundle.py`、`learning.py`、`recurring.py`、`calendar.py` 或 `networth.py` route shell；统计 read/exchange/Analyzer 不再保留 Flask `statistics` route shell，洞察异常不再保留 Flask `insights.py` route handler，剩余 Python statistics-adjacent 运行时代码只保留非 HTTP 报告文件生成。
