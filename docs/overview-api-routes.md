...
- 主要 Python REST route 文件当前按同名 package 组织：`auth/`、`statistics/`、`backup/`、`llm/` 保留原 `bp` 导出与 URL/method 契约；学习中心、账户、分类、分类规则、标签、模板、设置包、预算、账单、matching、recurring suggestions、calendar 与 networth REST 主链由 Rust runtime 提供，不再保留 Flask `accounts/` / `categories/` / `budgets/` / `bills/` / `matching/` route package、`category_rules.py`、`tags.py`、`templates.py`、`settings_bundle.py`、`learning.py`、`recurring.py`、`calendar.py` 或 `networth.py` route shell；统计 read/exchange/Analyzer 不再保留 Flask `statistics` route shell，洞察异常不再保留 Flask `insights.py` route handler，剩余 Python statistics-adjacent 运行时代码只保留非 HTTP 报告文件生成。
