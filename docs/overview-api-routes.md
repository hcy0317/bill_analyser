...
- 分类规则列表 `GET /api/category-rules/` 已由 Rust taxonomy runtime 读取 `category_rules` 与 `categories`，保留 Flask-compatible `success/data/total` envelope、当前用户隔离、`category_id` 筛选和 `enabled_only=false` 查询语义；分类规则创建、更新、删除、test、defaults、migrate、reorder、engine reload、matcher 执行和批量分类仍在 Python `CategoryEngine` / `KeywordMatcher` 内完成。
