...
- 账单/分类路由层已完成一轮适配器收敛，账户 Flask route package 后续已删除：
  - `bills/` 已拆分基础上下文与 adapter 上下文，非转换型路由不再默认构造交易适配器；
  - `categories/` / `bills/` 已统一改从中性适配器模块导入。
...
- 主要 Python REST route 文件当前按同名 package 组织：`bills/`、`auth/`、`statistics/`、`categories/`、`budgets/`、`backup/`、`matching/`、`llm/` 均保留原 `bp` 导出与 URL/method 契约，内部按 CRUD、查询、导入、候选、配置、用户数据等功能域拆分；账户 REST 主链由 Rust taxonomy runtime 提供，不再保留 Flask `accounts/` route package。
