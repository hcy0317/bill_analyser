...
- 主要 Python REST route 文件当前按同名 package 组织：`auth/`、`statistics/`、`backup/`、`matching/`、`llm/` 均保留原 `bp` 导出与 URL/method 契约；账户、分类、分类规则、标签、模板、设置包、预算与账单 REST 主链由 Rust runtime 提供，不再保留 Flask `accounts/` / `categories/` / `budgets/` / `bills/` route package、`category_rules.py`、`tags.py`、`templates.py` 或 `settings_bundle.py` route shell；统计 read/exchange/Analyzer 不再保留 Flask `statistics` route shell，洞察异常不再保留 Flask `insights.py` route handler，剩余 Python statistics-adjacent 运行时代码聚焦 networth、calendar 与非 HTTP 报告文件生成。
