...
- 主要 Python REST route 文件当前按同名 package 组织：`bills/`、`auth/`、`statistics/`、`budgets/`、`backup/`、`matching/`、`llm/` 均保留原 `bp` 导出与 URL/method 契约，内部按 CRUD、查询、导入、候选、配置、用户数据等功能域拆分；账户与分类 REST 主链由 Rust taxonomy runtime 提供，不再保留 Flask `accounts/` / `categories/` route package。
