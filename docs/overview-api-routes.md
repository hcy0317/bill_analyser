...
- Rust 主 HTTP 服务在 `import_db_runtime` 模式下接管核心账单/交易 CRUD 写入口、legacy modify/delete、交易图片、账单导出、正式账单 recurring 匹配、账户对账单和账单分类 quick actions：
  ...
  - Python-proxied：无 bills route set 内剩余 CRUD/list/export/pictures/recurring、legacy modify/delete、分类 action 或 reconciliation 代理项，旧 Flask `bills/crud_prepare.py`、`bills/crud_create_update.py`、`bills/crud_query.py`、`bills/category_actions.py` 与 `bills/reconciliation.py` route shell 已删除；Python bills sidecar 只保留导入、配置、学习、v2 session/preview/decision 等 import 路径，包括 `/api/bills/import/batch`；未登记的 `/api/bills/pictures/*` 子路径在 manifest fallback 下返回 404；
...
- Rust 主 HTTP 服务在 `import_db_runtime` 模式下接管无外部 provider 依赖的统计读取入口与 Analyzer 统计聚合入口：
  ...
  - Rust-owned Analyzer：`GET /api/statistics/overview`、`GET /api/statistics/trends`、`GET /api/statistics/comparison`、`GET /api/statistics/category`、`GET /api/statistics/trend`；
  ...
  - Python-proxied：无 statistics route package 内剩余 read/analyzer/exchange 代理项；旧 Flask `statistics` read/exchange/Analyzer route shell 已删除，Flask sidecar 仅保留空 `statistics.bp` 作为 package 兼容出口；
