...
- Rust 主 HTTP 服务在 `import_db_runtime` 模式下接管备份任务配置入口：
  - Rust-owned：`GET /api/backup/jobs`、`POST /api/backup/jobs`；
  - Python-proxied：`GET /api/backup/`、`POST /api/backup/create`、`POST /api/backup/restore/verify`、`GET /api/backup/download/<filename>`、`DELETE /api/backup/delete/<filename>`、`POST /api/backup/restore/<filename>`、`POST /api/backup/cleanup` 仍由 Flask backup package 提供；
  - Rust backup jobs runtime 复用 `ops` 的 job payload 校验，按当前 `user_id` 读写 `backup_jobs`，同一用户同一 `job_type` 保存会更新既有任务，跨用户或不存在的 `id` 不会被更新，并写入包含 actor 与来源 IP 的 `backup_job_saved` 审计；文件备份、zip/Fernet、restore 和 cleanup 的实际文件系统动作仍留在后续 P12 ops 切面。
...
- 主要 Python REST route 文件当前按同名 package 组织：`auth/`、`statistics/`、`backup/`、`llm/` 保留原 `bp` 导出与尚未迁移的 URL/method 契约；学习中心、账户、分类、分类规则、标签、模板、设置包、预算、账单、matching、recurring suggestions、calendar、networth 与 backup jobs REST 主链由 Rust runtime 提供，不再保留 Flask `accounts/` / `categories/` / `budgets/` / `bills/` / `matching/` route package、`category_rules.py`、`tags.py`、`templates.py`、`settings_bundle.py`、`learning.py`、`recurring.py`、`calendar.py` 或 `networth.py` route shell；Flask backup package 仍提供备份文件列表、创建、恢复预验证、下载、删除、恢复和清理；统计 read/exchange/Analyzer 不再保留 Flask `statistics` route shell，洞察异常不再保留 Flask `insights.py` route handler，剩余 Python statistics-adjacent 运行时代码只保留非 HTTP 报告文件生成。
