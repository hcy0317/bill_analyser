...
- `api/routes/backup/`、`api/routes/encryption.py`、`api/routes/auth/user_data.py`：备份/恢复仍由 Flask route shell + Python 文件/DB 操作执行；`GET /api/settings/encryption/status` 在默认 `import_db_runtime` 下已由 Rust taxonomy/settings runtime 直接响应 SQLCipher 状态投影，真实 SQLCipher 连接 patch 与迁移仍由 Python/后续 ops 切面保留；用户数据统计、CSV/TSV 导出与数据清理已由 Rust auth runtime 直接接管。备份恢复会拒绝包含绝对路径或 `..` 段的 zip member，Rust `ops.rs` 已固定安全文件名、备份摘要、cleanup 计划、Fernet key 派生、SQLCipher 状态、用户数据统计/审计 payload 与 sync/report 格式合同。
