...
- `database/bills/` / `database/categories/` / `database/category_rules/` / `database/accounts/` / `database/tags/` / `database/templates/` / `database/settings_bundle/`：账单、分类、分类规则、账户、标签、模板域的 CRUD、批量操作、设置包导入导出与查询辅助；账户和设置包域已按读取/变更/余额、section export/import/upsert/resolution 拆分，账户 master-data list/get/create/update/delete/subAccounts/display-order、标签 master-data CRUD/display-order、分类 master-data list/get/create/batch-ensure/update/delete/parent-group rename、模板 master-data list/get/create/update/delete/display-order 与 enabled recurring 读取在普通文件库上分别经 `core/account_rust_bridge.py`、`core/tag_rust_bridge.py`、`core/category_rust_bridge.py`、`core/template_rust_bridge.py` 调用 Rust `bill_taxonomy_bridge`；settings bundle taxonomy sections 的 schema/section normalization、账户/分类/标签字段规范化、模板引用解析和导出 DTO 构建经 `core/settings_bundle_rust_bridge.py` 调用 Rust helper，但导入 SQL 仍由 Python 在原 `aiosqlite` 事务里执行。`:memory:`、SQLCipher、账户余额/交易批量操作、`bill_tags` 关联 helper、分类统计、分类规则持久化、模板 recurring 匹配/绑定推进、settings bundle category-rule/LLM/OCR 导入与 recurring suggestion/import-flow 写路径保留 Python 路径。
- `database/users/auth/` / `database/users/data/` / `database/audit_backup/`：用户与会话、2FA 与应用设置、用户数据管理、审计/备份域持久化逻辑
- `database/budgets/core/` / `database/budgets/execution/` / `database/budgets/forecast/` / `database/budgets/reporting/`：预算主数据、分类上下文与分组 helper，以及执行统计 / 历史 / 预测 / 导入导出查询；预算分类上下文会把历史 `categories.type=1` 归一为当前支出类型 `3`，避免旧分类预算在执行统计和预算列表中被类型过滤漏掉。一级/二级分类预算与月度/季度/年度预算都按父子预算关系上卷：子集总额不超过父级时保留父级金额，超过时父级金额提升为子集总和；历史预算读取会在无账单支出时按需返回预算行，并在支出/投资类型之间保持隔离。预算域的期间解析、执行汇总去重、父级上卷、历史窗口和预测计算合同已由 Rust `crates/bill-analyser-core/src/budgets.rs` 固定；Flask route、aiosqlite 持久化和 snapshot/import/export 写路径仍由 Python 运行时负责。
