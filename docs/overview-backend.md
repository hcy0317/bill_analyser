...
- `bills/`：账单同名 package；按 CRUD/pictures、通用导入解析、导入配置、导入学习、reconciliation、v2 session/preview/decision 等功能域拆分，`bills.bp` 和 `/api/bills/*` 契约保持稳定；批量更新/删除与分类 quick actions 已由 Rust bills runtime 接管，旧 Flask `bills/category_actions.py` route shell 已删除
