...
- `database/bills/` / `database/categories/` / `database/category_rules/` / `database/accounts/` / `database/tags/` / `database/templates/` / `database/settings_bundle/`：账单、分类、分类规则、账户、标签、模板域的 CRUD、批量操作、设置包导入导出与查询辅助；账户和设置包域已按读取/变更/余额、section export/import/upsert/resolution 拆分，账户 master-data list/get/create/update/delete/subAccounts/display-order、标签 master-data CRUD/display-order、分类 master-data list/get/create/batch-ensure/update/delete/parent-group rename、分类账单统计、分类规则列表/创建/更新/删除/重排/defaults/migrate/测试、规则中心概览、设置包 JSON 导出、模板 master-data list/get/create/update/delete/display-order 与 enabled recurring 读取在普通文件库上分别经 Rust taxonomy runtime 或 `core/account_rust_bridge.py`、`core/tag_rust_bridge.py`、`core/category_rust_bridge.py`、`core/template_rust_bridge.py` 调用 Rust `bill_taxonomy_bridge`；settings bundle taxonomy sections 的 schema/section normalization、账户/分类/标签字段规范化、模板引用解析和导出 DTO 构建经 `core/settings_bundle_rust_bridge.py` 调用 Rust helper，Rust HTTP 导出会组合这些 sections、分类规则、LLM 配置骨架和 OCR 配置，但导入 SQL 仍由 Python 在原 `aiosqlite` 事务里执行。`:memory:`、SQLCipher、账户余额/交易批量操作、`bill_tags` 关联 helper、legacy `GET|PUT /api/categories/rules` config/cache、模板 recurring 匹配/绑定推进、settings bundle category-rule/LLM/OCR 导入与 recurring suggestion/import-flow 写路径保留 Python 路径。
