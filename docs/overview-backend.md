...
- `database/bills/` / `database/categories/` / `database/category_rules/` / `database/accounts/` / `database/tags/` / `database/templates/` / `database/settings_bundle/`：账单、分类、分类规则、账户、标签、模板域的 CRUD、批量操作、设置包导入导出与查询辅助；账户和设置包域已按读取/变更/余额、section export/import/upsert/resolution 拆分，账户 master-data list/get/create/update/delete/subAccounts/display-order、标签 master-data CRUD/display-order、分类 master-data list/get/create/batch-ensure/update/delete/parent-group rename、模板 master-data list/get/create/update/delete/display-order 与 enabled recurring 读取在普通文件库上分别经 `core/account_rust_bridge.py`、`core/tag_rust_bridge.py`、`core/category_rust_bridge.py`、`core/template_rust_bridge.py` 调用 Rust `bill_taxonomy_bridge`，`:memory:`、SQLCipher、账户余额/交易批量操作、`bill_tags` 关联 helper、分类统计、分类规则/匹配、模板 recurring 匹配/绑定推进、settings bundle taxonomy/template import/upsert 与 recurring suggestion/import-flow 写路径保留 Python 路径。
