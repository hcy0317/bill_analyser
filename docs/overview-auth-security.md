...
- Rust 迁移当前已具备认证安全核心 foundation，并通过预构建的 `bill_auth_bridge` / `bill_analyser.core.auth_rust_bridge` 在 `POST /api/tokens/refresh` 中接管 refresh token 解码后 claims 形状校验；`start_backend.ps1` 与后端 CI 会先构建该 bridge，其他部署方式需通过 `BILL_ANALYSER_RUST_AUTH_BRIDGE` 指向可执行文件。Python/Flask 仍负责 JWT 解码、访问/刷新 token 签发、session DB 写入、cloud settings 读取和 REST 响应外壳。Rust foundation 还覆盖 Bearer header 解析、API/MCP/session token 类型投影、UA 设备名投影、注册密码策略校验与 2FA 恢复码规范化；密码哈希、JWT 签名/验证、2FA/profile/token DB 写入与审计写入尚未切到 Rust。
