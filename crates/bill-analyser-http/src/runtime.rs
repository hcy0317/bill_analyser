    ...
    pub fn for_import_route_mode(import_route_mode: ImportRouteMode) -> Self {
        let (runtime_boundary, business_migration) = match import_route_mode {
            ...
            ImportRouteMode::ImportDbRuntime => (
                "rust-http-shell:import-db-runtime+bills-crud-runtime+budgets-crud-execution-forecast-history-import-runtime+statistics-read-runtime+auth-login-register-token-account-recovery-profile-cloud-external-auth-user-data-statistics-runtime",
                "import-db-runtime+bills-crud-runtime+budgets-crud-execution-forecast-history-import-runtime+statistics-read-runtime-partial+auth-login-register-token-session-personal-refresh-logout-account-recovery-oauth2-authorize-profile-cloud-external-auth-system-user-data-statistics-runtime",
            ),
        };
...
pub fn http_shell_health(config: &HttpShellConfig) -> HttpShellHealth {
    ...
    if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
        ...
        details.insert(
            ...
            "owned login, registration, token session list/revoke, API/MCP personal token generation, token refresh, logout, account-recovery email verification/resend/password forgot/reset, profile, avatar, profile cloud settings, profile external-auth list/unlink, profile verification resend, OAuth2 authorize disabled-safe/not-implemented, system version, and user-data statistics routes; 2FA, real OAuth provider exchange, step-up, data export, and destructive data-clear routes proxied".to_string(),
        );
    }
    details.insert(
