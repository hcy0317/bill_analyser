...
pub fn http_shell_health(config: &HttpShellConfig) -> HttpShellHealth {
    ...
    if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
        ...
        details.insert(
            ...
            "owned backup file list/create/download/delete/restore/verify/cleanup and job list/save routes backed by Rust zip/Fernet file I/O, SQLite backup_ops schema, backup_records updates, safe restore validation, and backup audit log writes".to_string(),
        );
    }
    details.insert(
