...
pub fn http_shell_health(config: &HttpShellConfig) -> HttpShellHealth {
    ...
    if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
        ...
        details.insert(
            ...
            "owned core bills/transactions CRUD, transaction picture upload/unused cleanup, bills CSV/XLSX export, and recurring candidates/match; reconciliation/category actions proxied".to_string(),
        );
        details.insert(
