...
pub fn http_shell_health(config: &HttpShellConfig) -> HttpShellHealth {
    ...
    details.insert(
        ...
        if config.import_route_mode.intercepts_import_routes() {
            if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
                "/api/health,/api/runtime,import/preview-adjacent runtime routes,bills CRUD runtime routes,bills picture runtime routes,bills export runtime route,bills recurring runtime routes,bills reconciliation runtime route,budgets CRUD/execution/forecast/history/import runtime routes,statistics read runtime routes,auth login/register/token/account-recovery/profile/cloud/external-auth/system/user-data-statistics-export-clear/2fa-status/2fa-verify/2fa-recovery-verify/2fa-write/step-up runtime routes".to_string()
            } else {
    ...
    if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
        ...
        details.insert(
            ...
            "owned core bills/transactions CRUD, transaction picture upload/unused cleanup, bills CSV/XLSX export, recurring candidates/match, and reconciliation statements; category actions proxied".to_string(),
        );
        details.insert(
