...
pub fn http_shell_health(config: &HttpShellConfig) -> HttpShellHealth {
    ...
    if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
        ...
        details.insert(
            "backup_jobs_runtime".to_string(),
            "owned backup job list/save routes backed by Rust SQLite backup_ops schema and backup audit log writes; backup file list/create/download/delete/restore/verify/cleanup remain Python-proxied until the file-system and encryption cutover completes".to_string(),
        );
    }
    details.insert(
