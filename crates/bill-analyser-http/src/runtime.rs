    ...
    pub fn for_import_route_mode(import_route_mode: ImportRouteMode) -> Self {
        let (runtime_boundary, business_migration) = match import_route_mode {
            ...
            ImportRouteMode::ImportDbRuntime => (
                "rust-http-shell:import-db-runtime+bills-crud-runtime+bills-picture-runtime+bills-export-runtime+bills-recurring-runtime+bills-reconciliation-runtime+bills-category-actions-runtime+budgets-crud-execution-forecast-history-import-runtime+statistics-read-runtime+statistics-exchange-runtime+taxonomy-accounts-runtime+taxonomy-tags-runtime+taxonomy-tags-batch-runtime+taxonomy-categories-runtime+taxonomy-templates-runtime+taxonomy-settings-bundle-runtime+auth-login-register-token-account-recovery-profile-cloud-external-auth-user-data-statistics-2fa-status-verify-recovery-write-step-up-export-clear-runtime",
...
pub fn http_shell_health(config: &HttpShellConfig) -> HttpShellHealth {
    ...
    details.insert(
        ...
        if config.import_route_mode.intercepts_import_routes() {
            if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
                "/api/health,/api/runtime,import/preview-adjacent runtime routes,bills CRUD runtime routes,bills picture runtime routes,bills export runtime route,bills recurring runtime routes,bills reconciliation runtime route,bills category actions runtime routes,budgets CRUD/execution/forecast/history/import runtime routes,statistics read/exchange runtime routes,taxonomy account CRUD/display-order/sync-balances/transaction-action runtime routes,taxonomy tag CRUD/display-order runtime routes,taxonomy category master-data/statistics/category-rule-list-test/rule-overview/settings-bundle import-export runtime routes,taxonomy templates CRUD/display-order runtime routes,auth login/register/token/account-recovery/profile/cloud/external-auth/system/user-data-statistics-export-clear/2fa-status/2fa-verify/2fa-recovery-verify/2fa-write/step-up runtime routes".to_string()
            } else {
    ...
    if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
        ...
        details.insert(
            ...
            "owned DB-backed category statistics, category trends, asset trends, category pie, top merchants, transaction amounts, exchange provider fallback, and user custom exchange rates; analyzer overview/comparison/trends/category remains proxied".to_string(),
        );
        details.insert(
