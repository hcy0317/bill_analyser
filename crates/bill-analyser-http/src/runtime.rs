...
pub fn http_shell_health(config: &HttpShellConfig) -> HttpShellHealth {
    ...
    if config.import_route_mode == ImportRouteMode::ImportDbRuntime {
        ...
        details.insert(
            ...
            "owned account list/detail/create/update/delete/display-order routes with frontend cents to SQLite yuan conversion; transaction move/clear and balance sync remain proxied".to_string(),
        );
        details.insert(
        ...
        details.insert(
            ...
            "owned category list/tree/flat/detail/create/update/delete/batch/move/import/export/all/statistics routes plus category-rule list/test and rule overview routes with user-scoped DB reads and writes; category rule mutations and settings bundle import remain proxied".to_string(),
        );
        details.insert(
            "taxonomy_templates_runtime".to_string(),
            "owned template list/detail/create/update/delete/display-order routes with user-scoped DB reads and writes; recurring matching/binding and settings bundle import remain proxied".to_string(),
        );
        details.insert(
